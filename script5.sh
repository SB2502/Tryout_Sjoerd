...
cat nytimes_humanities_tabbed.csv | awk -F '\t' '{if ($2 ~ "Front Page" || $8 ~ "Front Page") print $1}' | sed -E 's/-..T..:..:..Z//'| awk -F "-" '{print $1 "\t" $2}' | sort | uniq -c | awk '{print "\t" $1 "\t" $2 "\t" $3}' > maanden_per_jaartal.txt

#make it look nice

echo -e "Nr. of Articles\tYear\tMonth" "\n" "$(cat maanden_per_jaartal.txt)"
...
while true; do
    ...
    case $yn in
        ...
        [Yy]* ) cat nytimes_humanities_tabbed.csv | awk -F '\t' '{if ($2 ~ "Front Page" || $8 ~ "Front Page") print $1}' | sed -E 's/-..T..:..:..Z//'| awk -F "-" '{print $1 "\t" $2}' | sort | uniq -c | awk '{print "\t" $1 "\t" $2 "\t" $3}' | sort -nr | sed 5q > maanden_per_jaartal_top5.txt
				echo -e "Nr. of Articles\tYear\tMonth" "\n" "$(cat maanden_per_jaartal_top5.txt)"; break;;
...
rm maanden_per_jaartal.txt
rm maanden_per_jaartal_top5.txt
